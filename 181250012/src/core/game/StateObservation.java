...
public class StateObservation {
    ...
    /*判断两个状态是否相等*/
    public boolean eqq(StateObservation ab){
        ArrayList<Observation>[][] this_grid = this.getObservationGrid();
        ArrayList<Observation>[][] cmp_grid = ab.getObservationGrid();
        boolean check = true;
        for(int i = 0; i < 5; i ++){
            if(check == false) break;
            for(int j = 0; j < 6; j ++){
                if(check == false) break;
                if(this_grid[i][j].size() != cmp_grid[i][j].size()) {check = false; break;}
                if(this_grid[i][j].size() != 0){
                    for(int k = 0; k < this_grid[i][j].size();k++){
                        if(this_grid[i][j].get(k).itype != cmp_grid[i][j].get(k).itype) {
                            check = false;
                            break;
                        }
                    }
